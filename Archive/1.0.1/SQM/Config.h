// Config.h 
// User specific configuration for SQM
// 
// Copyright (c) 2019 Roman Hujer   http://hujer.net
//

// Select only one your display
#define SSD1306_ON // SSD1306 0.96" 128x64 OLED display 
#define SH1106_OFF   // SH1106  1.3"  128x64 OLED display

#define SQM_W_ON     //SQM_W protocol 
#define SQM_LU_OFF   //SQM_LU (SQM-LU-DL) protocol 

//
#define TEMPER_CALIB_ON         // Temperature calibration
#define CALIBRATION_OFFSET 0.0  // SQM Calibration offset

#define BUZZER_ON  // Buzzer

#define DEBUG_ON   // Debug

#define EXTENDET_PROTOCOL_ON  // My Extender protocol for weather info 

#define STATUS_LED_OFF         // Hardware Version 1.x no status LED
